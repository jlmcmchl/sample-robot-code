...
public class Pose2d implements IPose2d<Pose2d> {
  ...
  private final static double DEFAULT_MAX_ERROR = 1E-9;
  ...
  public static Pose2d exp(final Twist2d delta) {
    ...
    return new Pose2d(new Translation2d(delta.deltaX * s - delta.deltaY * c, delta.deltaX * c + delta.deltaY * s),
  ...
  public static Twist2d log(final Pose2d transform) {
    ...
    if (Math.abs(cos_minus_one) < DEFAULT_MAX_ERROR) {
      ...
      halftheta_by_tan_of_halfdeltaTheta = -(half_deltaTheta * transform.getRotation().sin()) / cos_minus_one;
    }
  ...
  public boolean isColinear(final Pose2d other) {
    ...
    return (MathUtils.epsilonEquals(twist.deltaY, 0.0) && MathUtils.epsilonEquals(twist.deltaTheta, 0.0));
  }
  ...
  public Rotation2d rotationTo(Pose2d other) {
    return getRotation().inverse().rotateBy(
        getTranslation().inverse().translateBy(other.getTranslation()).direction());
  }
}

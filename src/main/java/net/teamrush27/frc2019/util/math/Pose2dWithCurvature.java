...
public class Pose2dWithCurvature implements IPose2d<Pose2dWithCurvature>, ICurvature<Pose2dWithCurvature> {
  protected static final Pose2dWithCurvature IDENTITY = new Pose2dWithCurvature();
  ...
  public Pose2dWithCurvature interpolate(final Pose2dWithCurvature other, double x) {
    return new Pose2dWithCurvature(getPose().interpolate(other.getPose(), x),
        MathUtils.interpolate(getCurvature(), other.getCurvature(), x),
        MathUtils.interpolate(getDCurvatureDs(), other.getDCurvatureDs(), x));
  }
  ...
  public boolean equals(final Object other) {
    ...
    return getPose().equals(p2dwc.getPose()) && MathUtils.epsilonEquals(getCurvature(), p2dwc.getCurvature()) && MathUtils.epsilonEquals(getDCurvatureDs(), p2dwc.getDCurvatureDs());
  }
  ...
  public String header(String base) {
    return getPose().header(base) + "," + base + "_curv," + base + "_dcurv";
  }
}
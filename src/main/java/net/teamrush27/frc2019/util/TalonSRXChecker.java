...
public class TalonSRXChecker extends MotorChecker<TalonSRX> {
  ...
  public static boolean checkMotors(Subsystem subsystem, ArrayList<MotorConfig<TalonSRX>> motorsToCheck,
  ...
  protected void restoreConfiguration() {
    for (int i = 0; i < mMotorsToCheck.size(); ++i) {
      mMotorsToCheck.get(i).mMotor.set(mStoredConfigurations.get(i).mMode, mStoredConfigurations.get(i).mSetValue);
    }
  }

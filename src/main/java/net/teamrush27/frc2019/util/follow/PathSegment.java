...
public class PathSegment {
     ...
     */
    public PathSegment(double startX, double startY, double endX, double endY, 
    ...
    public PathSegment(double startX, double startY, double endX, double endY, double maxSpeed, MotionState startState,
     ...
     */
    public PathSegment(double startX, double startY, double endX, double endY, 
    		double centerX, double centerY, double maxSpeed, MotionState startState, double endSpeed) {
        this.start = new Translation2d(startX, startY);
        this.end = new Translation2d(endX, endY);
    ...
    public PathSegment(double startX, double startY, double endX, double endY, double centerX, 
    		double centerY, double maxSpeed, MotionState startState, double endSpeed, String marker) {
        this.start = new Translation2d(startX, startY);
        this.end = new Translation2d(endX, endY);
        this.center = new Translation2d(centerX, centerY);
    ...
    public void createMotionProfiler(MotionState startState, double endSpeed) {
        MotionProfileConstraints motionConstraints = new MotionProfileConstraints(maxSpeed,
                FollowingConstants.MAX_ACCELERATION);
        MotionProfileGoal goalState = new MotionProfileGoal(getLength(), endSpeed);
        speedController = MotionProfileGenerator.generateProfile(motionConstraints, goalState, startState);
    ...
    public Translation2d getClosestPoint(Translation2d position) {
        if (isLine) {
            ...
            double closestPoint = ((position.x() - start.x()) * delta.x() + (position.y() - start.y()) * delta.y())
                    / (delta.x() * delta.x() + delta.y() * delta.y());
            if (closestPoint >= 0 && closestPoint <= 1)
                return new Translation2d(start.x() + closestPoint * delta.x(), start.y() + closestPoint * delta.y());

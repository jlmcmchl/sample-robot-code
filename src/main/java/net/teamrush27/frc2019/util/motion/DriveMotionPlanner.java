...
public class DriveMotionPlanner implements CSVWritable {
  ...
  public Trajectory<TimedState<Pose2dWithCurvature>> generateTrajectory(
    ...
    if (reversed) {
      ...
      for (int i = 0; i < trajectory.length(); ++i) {
        flipped.add(
            new Pose2dWithCurvature(trajectory.getState(i).getPose().transformBy(flip), -trajectory
    ...
    Trajectory<TimedState<Pose2dWithCurvature>> timed_trajectory = TimingUtil
        .timeParameterizeTrajectory
            (reversed, new
                    DistanceView<>(trajectory), kMaxDx, all_constraints, start_vel, end_vel, max_vel,
                max_accel);
  ...
  public static class Output {
    ...
    public String toString() {
      DecimalFormat fmt = new DecimalFormat("#0.000");
      return String.format("(%s, %s) (%s, %s) (%s, %s)", fmt.format(left_velocity),
          fmt.format(right_velocity), fmt.format(left_accel), fmt.format(right_accel),
          fmt.format(left_feedforward_voltage), fmt.format(right_feedforward_voltage));
    }
  }
  ...
  protected Output updatePID(DriveDynamics dynamics, Pose2d current_state) {
    ...
    adjusted_velocity.angular =
        dynamics.chassis_velocity.angular + dynamics.chassis_velocity.linear * kPathKY *
            Units.inches_to_meters(mError.getTranslation().y()) + kPathKTheta * mError.getRotation()
            .getRadians();
    ...
    final WheelState wheel_velocities = mModel
        .solveInverseKinematics(adjusted_velocity);
    final double left_voltage =
        dynamics.voltage.left + (wheel_velocities.left - dynamics.wheel_velocity
            .left) / mModel.left_transmission().speed_per_volt();
    final double right_voltage =
        dynamics.voltage.right + (wheel_velocities.right - dynamics.wheel_velocity
            .right) / mModel.right_transmission().speed_per_volt();

    return new Output(wheel_velocities.left, wheel_velocities.right,
        dynamics.wheel_acceleration.left, dynamics
  ...
  protected Output updateNonlinearFeedback(DifferentialDrive.DriveDynamics dynamics,
    ...
    final double kBeta = 1.0;  // >0.
    final double kZeta = 0.5;  // Damping coefficient, [0, 1].

    // Compute gain parameter.
    final double k =
        2.0 * kZeta * Math.sqrt(kBeta * dynamics.chassis_velocity.linear * dynamics.chassis_velocity
    ...
    dynamics.chassis_acceleration.angular =
        mDt == 0 ? 0.0 : (dynamics.chassis_velocity.angular - prev_velocity_
            .angular) / mDt;
    ...
    return new Output(dynamics.wheel_velocity.left, dynamics.wheel_velocity.right,
        dynamics.wheel_acceleration
            .left, dynamics.wheel_acceleration.right, feedforward_voltages.left,
  ...
  public Output update(double timestamp, Pose2d current_state) {
    ...
    if (!mCurrentTrajectory.isDone()) {
      ...
      final DriveDynamics dynamics = mModel.solveInverseDynamics(

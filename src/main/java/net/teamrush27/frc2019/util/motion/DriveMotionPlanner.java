...
public class DriveMotionPlanner implements CSVWritable {
  ...
  IDrivetrainModel.ChassisState prev_velocity_ = new IDrivetrainModel.ChassisState();
  double mDt = 0.0;

  public DriveMotionPlanner() {
    final DCMotorTransmission transmission = new DCMotorTransmission(
        1.0 / ChezyConstants.kDriveKv,
        Units.inches_to_meters(ChezyConstants.kDriveWheelRadiusInches) * Units.inches_to_meters(ChezyConstants
            .kDriveWheelRadiusInches) * ChezyConstants.kRobotLinearInertia / (2.0 * ChezyConstants.kDriveKa),
    ...
    mModel = new DifferentialDrive(
        ...
        Units.inches_to_meters(ChezyConstants.kDriveWheelTrackWidthInches / 2.0 * ChezyConstants.kTrackScrubFactor),
  ...
  public Trajectory<TimedState<Pose2dWithCurvature>> generateTrajectory(
    ...
    final TimingConstraint<Pose2dWithCurvature> drive_constraints = new
  ...
  protected Output updatePID(IDrivetrainModel.DriveDynamics dynamics, Pose2d current_state) {
    IDrivetrainModel.ChassisState adjusted_velocity = new IDrivetrainModel.ChassisState();
    ...
    final IDrivetrainModel.WheelState wheel_velocities = mModel.solveInverseKinematics(adjusted_velocity);
  ...
  protected Output updatePurePursuit(IDrivetrainModel.DriveDynamics dynamics, Pose2d current_state) {
    ...
    IDrivetrainModel.ChassisState adjusted_velocity = new IDrivetrainModel.ChassisState();
    // Feedback on longitudinal error (distance).
    adjusted_velocity.linear = dynamics.chassis_velocity.linear + ChezyConstants.kPathKX * Units.inches_to_meters
        (mError.getTranslation().x());
  ...
  protected Output updateNonlinearFeedback(IDrivetrainModel.DriveDynamics dynamics, Pose2d current_state) {
    ...
    final IDrivetrainModel.ChassisState adjusted_velocity = new IDrivetrainModel.ChassisState(
    ...
    IDrivetrainModel.WheelState feedforward_voltages = mModel.solveInverseDynamics(dynamics.chassis_velocity,
  ...
  public Output update(double timestamp, Pose2d current_state) {
    ...
    if (!mCurrentTrajectory.isDone()) {
      ...
      final IDrivetrainModel.DriveDynamics dynamics = mModel.solveInverseDynamics(
          new IDrivetrainModel.ChassisState(velocity_m, velocity_m * curvature_m),
          new IDrivetrainModel.ChassisState(acceleration_m,

...
public abstract class MotorChecker<T> {
  ...
  protected boolean checkMotorsImpl(Subsystem subsystem, ArrayList<MotorConfig<T>> motorsToCheck,
      CheckerConfig checkerConfig) {
    boolean failure = false;
    System.out.println("////////////////////////////////////////////////");
    System.out.println("Checking subsystem " + subsystem.getClass() + " for " + motorsToCheck.size() + " motors.");
    ...
    if (currents.size() > 0) {
      ...
      if (!MathUtils.allCloseTo(currents, average, checkerConfig.mCurrentEpsilon)) {
    ...
    if (rpms.size() > 0) {
      ...
      if (!MathUtils.allCloseTo(rpms, average, checkerConfig.mRPMEpsilon)) {

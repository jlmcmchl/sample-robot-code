...
public class CANTalonFactory {
    ...
    public static TalonSRX createTalon(int id, Configuration config) {
        ...
        talon.configPeakOutputForward(config.MAX_OUTPUT_VOLTAGE / config.PEAK_VOLTAGE, config.CONFIG_TIMEOUT_MS);
        talon.configPeakOutputReverse(-(config.MAX_OUTPUT_VOLTAGE / config.PEAK_VOLTAGE), config.CONFIG_TIMEOUT_MS);
        ...
        talon.configVelocityMeasurementPeriod(config.VELOCITY_MEASUREMENT_PERIOD, config.CONFIG_TIMEOUT_MS);
        ...
        talon.configForwardLimitSwitchSource(LimitSwitchSource.Deactivated, LimitSwitchNormal.Disabled, config.CONFIG_TIMEOUT_MS);
		talon.configReverseLimitSwitchSource(LimitSwitchSource.Deactivated, LimitSwitchNormal.Disabled, config.CONFIG_TIMEOUT_MS);
        ...
        talon.configVelocityMeasurementPeriod(config.VELOCITY_MEASUREMENT_PERIOD, config.CONFIG_TIMEOUT_MS);
        talon.configVelocityMeasurementWindow(config.VELOCITY_MEASUREMENT_ROLLING_AVERAGE_WINDOW, config.CONFIG_TIMEOUT_MS);

        talon.configOpenloopRamp(config.VOLTAGE_RAMP_RATE, config.CONFIG_TIMEOUT_MS);

        talon.setStatusFramePeriod(StatusFrameEnhanced.Status_1_General, config.GENERAL_STATUS_FRAME_RATE_MS, config.CONFIG_TIMEOUT_MS);
        talon.setStatusFramePeriod(StatusFrameEnhanced.Status_2_Feedback0, config.FEEDBACK_STATUS_FRAME_RATE_MS, config.CONFIG_TIMEOUT_MS);
        ...
        talon.setStatusFramePeriod(StatusFrameEnhanced.Status_4_AinTempVbat, config.ANALOG_TEMP_VBAT_STATUS_FRAME_RATE_MS, config.CONFIG_TIMEOUT_MS);
        talon.setStatusFramePeriod(StatusFrameEnhanced.Status_8_PulseWidth, config.PULSE_WIDTH_STATUS_FRAME_RATE_MS, config.CONFIG_TIMEOUT_MS);
